...
DecisionAgentTemplate = NodeTemplate(
    ...
    instructions = (
        ...
        "Se lo {state} dell'immagine è 'invalid image' o 'unusable image', non devi dare nessuna raccomandazione. In questo caso devi limitarti a segnalare che l'immagine non è utilizzabile e chidere all'utente di fornire un'altra immagine. (Se lo {state} è 'unsusable image' è molto porbabile che sia per la qualità dell'immagine, se è 'invalid image' è perchè non rappresenta una foglia di vite, ma in entrambi i casi non è possibile procedere con una diagnosi e quindi con una raccomandazione.)"
