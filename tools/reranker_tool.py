...
class RerankerTool:
    def __init__(
        ...
        runtime: str = settings.RERANKER_RUNTIME,
        hf_model_name: str = settings.RERANKER_MODEL_NAME,
        openvino_model_path: str | Path = settings.RERANKER_MODEL_LOCAL_PATH,
        allow_hf_fallback: bool = settings.RERANKER_ALLOW_HF_FALLBACK,
        batch_size: int = settings.RERANKER_BATCH_SIZE,
        instruction: str = settings.RERANKER_INSTRUCTION,
    ) -> None:
        self.requested_model_name = model_name
        self.hf_model_name = hf_model_name
        self.openvino_model_path = Path(openvino_model_path)
        self.requested_torch_device = (torch_device or settings.RERANKER_TORCH_DEVICE or "auto").lower()
        self.requested_openvino_device = (openvino_device or settings.RERANKER_OPENVINO_DEVICE or "GPU").upper()
    ...
    def _select_runtime(self) -> RerankerRuntimeInfo:
            ...
            if not openvino_available:
                if not self.allow_hf_fallback:
                    raise FileNotFoundError(
                        "Modello reranker OpenVINO locale non trovato.\n"
                        f"Path atteso: {self.openvino_model_path}\n"
                        "Esegui prima: python scripts/export_reranker_openvino.py --verify\n"
                        "Oppure usa RERANKER_RUNTIME=auto/torch."
                    )
        ...
        if self.requested_runtime != "auto":
            raise ValueError(
                "RERANKER_RUNTIME non valido. Usa: auto, openvino, torch."
            )
    ...
    def _select_torch_runtime(self, details: str = "") -> RerankerRuntimeInfo:
    ...
    def _load_model(self, runtime_info: RerankerRuntimeInfo) -> CrossEncoder:
        processor_kwargs = {
    ...
    def rank(self, query: str, documents: Sequence[str]) -> list[RerankResult]:
        query = query.strip()
        ...
        results = [
            RerankResult(index=original_index, score=float(score))
            for original_index, score in zip(original_indices, scores)
        ]
